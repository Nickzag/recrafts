...
export async function validateReleaseCandidate(directory, { writeReport = true } = {}) {
  ...
  const checks = {
    ...
    commit_identities: /^[0-9a-f]{40}$/.test(manifest.artifact_source_commit) && /^[0-9a-f]{40}$/.test(manifest.release_evidence_commit) && /^[0-9a-f]{40}$/.test(manifest.post_review_evidence_commit),
    ...
    required_files: ["SKILL.md","manifest.json","runtime/interop_cli.mjs","runtime/owner_decision_import.mjs","contracts/host-analysis.schema.json","contracts/owner-decision.schema.json","fixtures/interop/host-analysis.fixture.json"].every((file) => inventoryText.includes(file)),
...
if (process.argv[1]?.endsWith("validate-r005-release-candidate.mjs")) {
  const directory = path.resolve(process.argv[2] ?? "release-candidates/recrafts-0.3.0-rc.1-build5");
