...
const [packageDirectory, metricsFile, coverageFile, reviewFile, outputFile] = process.argv.slice(2);
if (!packageDirectory || !metricsFile || !coverageFile || !reviewFile || !outputFile) throw new Error("Usage: node scripts/run-r003-preflight.mjs <package-dir> <metrics.json> <critical-coverage.json> <review.md> <output.json>");
const packageData = await loadExtractionPackage(packageDirectory);
const metrics = JSON.parse(await readFile(metricsFile, "utf8"));
const criticalCoverage = JSON.parse(await readFile(coverageFile, "utf8"));
...
const result = evaluateRealizationPreflight({ packageData, metrics, criticalCoverage, ownerReview: { verdict } });
