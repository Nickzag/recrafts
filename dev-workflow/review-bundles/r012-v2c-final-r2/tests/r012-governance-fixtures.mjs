...
export async function gateReports({ candidate, designSource = canonical } = {}) {
  ...
  const gateB = await evaluateDesignCoherence({ candidate, designSource, previewHtml: preview.html, browserEvidence: { status: "PASS", preview_sha256: sha(preview.html), screenshots: { desktop: { file: path.join(tmpDir, "desktop.png"), sha256: dHash }, compact: { file: path.join(tmpDir, "compact.png"), sha256: cHash }, mobile: { file: path.join(tmpDir, "mobile.png"), sha256: mHash } }, computed_styles: { root_tokens: {} }, _evidence_dir: tmpDir } });
  const gateABase = {
