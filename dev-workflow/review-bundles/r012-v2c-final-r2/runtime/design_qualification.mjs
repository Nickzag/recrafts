...
export function validateCandidateIsolation(runs, { canonicalAllowedInputManifest = null, manifestSha256 = null } = {}) {
  ...
  const failures = [];
  if (canonicalAllowedInputManifest && manifestSha256) {
    const actual = createHash("sha256").update(JSON.stringify(canonicalAllowedInputManifest)).digest("hex");
    if (actual !== manifestSha256) failures.push("canonical-manifest-tampered");
  }
  ...
  for (const run of runs) {
    ...
    if (!canonicalAllowedInputManifest) { failures.push(`${run.candidate_id}:missing-authority-manifest`); }
    const allow = canonicalAllowedInputManifest ? new Set(canonicalAllowedInputManifest.allowed_inputs) : new Set(run.allowed_inputs ?? []);
