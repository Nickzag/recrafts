...
test("Gate B derives a bound PASS from deep IR and actual Preview coverage", () => {
  ...
  const report = evaluateDesignCoherence({ candidate: candidate(canonical), designSource: canonical, previewHtml: preview.html });
...
test("Gate B rejects incomplete visual grammar, unknown governance, accessibility, and responsive semantics", () => {
  ...
  for (const source of mutations) {
    ...
    const report = evaluateDesignCoherence({ candidate: candidate(source), designSource: source, previewHtml: preview.html });
...
test("Gate B rejects stale Candidate and Preview bindings", () => {
  const preview = renderDesignPreview({ designSource: canonical, generatedAt: "2026-08-09T00:00:00.000Z" });
  assert.equal(evaluateDesignCoherence({ candidate: { ...candidate(canonical), design_sha256: "0".repeat(64) }, designSource: canonical, previewHtml: preview.html }).verdict, "FAIL");
  assert.equal(evaluateDesignCoherence({ candidate: candidate(canonical), designSource: canonical, previewHtml: preview.html.replace("Document title", "Tampered") }).verdict, "FAIL");
});
