...
function componentStyles(ir) {
  return ir.components.flatMap((component) => {
    ...
    const slotOrder = layout.slot_order.map((part) => `[data-recrafts-part=\"${esc(part)}\"]`).join(", ");
    const selector = `[data-recrafts-specimen="${component.id}"]`;
    const base = `${selector}{${bindingCss(component.specimen.token_bindings)};border-style:solid;display:flex;${flowCss};${alignCss};gap:${gap}${slotOrder ? `}${selector} ${slotOrder}{order:0` : ""}}`;
...
function renderComponent(component, state = component.states[0], content = component.specimen.sample_content) {
  const parts = component.anatomy.map((part, index) => `<span data-recrafts-part="${esc(part)}">${esc(content[index % content.length] ?? part)}</span>`).join("");
...
function renderComposition(composition, components, viewport = "desktop") {
  const componentById = new Map(components.map((component) => [component.id, component]));
  const responsive = viewport === "desktop" ? {} : viewport === "compact" ? { hidden: ["inspector"] } : { hidden: ["sidebar", "inspector"] };
  const regionOrder = composition.layout.region_order.filter((id) => !responsive.hidden?.includes(id));
  const regions = composition.layout.region_order.map((regionId) => composition.regions.find((region) => region.id === regionId)).filter(Boolean).map((region) => {
    const hidden = responsive.hidden?.includes(region.id) ? ` data-recrafts-visibility="collapsed" style="display:none"` : ` data-recrafts-visibility="preserved"`;
    ...
    return `<section data-recrafts-region="${esc(region.id)}"${hidden}><h3>${esc(region.role)}</h3>${specimens}</section>`;
  }).join("");
  return `<div data-recrafts-rule="${esc(composition.id)}" data-recrafts-composition="${esc(composition.id)}" data-recrafts-viewport="${viewport}">${regions}</div>`;
}
...
function visualBody(ir) {
  ...
  const viewports = ["desktop", "compact", "mobile"].map((viewport) => `<article class="rc-review-section"><h3 class="rc-review-label">${viewport === "desktop" ? "Desktop viewport" : viewport === "compact" ? "Compact viewport" : "Mobile viewport"}</h3><div class="rc-viewport-frame"><div data-recrafts-visual-region>${ir.compositions.map((composition) => renderComposition(composition, ir.components, viewport)).join("")}</div></div></article>`).join("");
