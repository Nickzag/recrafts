...
export function evaluateDesignCoherence({ candidate, designSource, previewHtml, browserEvidence = null }) {
  const failures = [];
  const checks = {};
  ...
  try {
    ...
    checks.browser_bound_visual = browserEvidence ? pass(browserEvidence.status === "PASS" && browserEvidence.screenshots && Object.keys(browserEvidence.screenshots).length >= 3) : "NOT_RUN";
