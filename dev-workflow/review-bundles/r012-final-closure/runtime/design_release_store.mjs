...
export async function createDesignRelease({ storeDirectory, releaseId, version, parentReleaseId, candidate, designSource, gateAReport, gateBReport, decisionReceipt }) {
  ...
  if (!gateAProd) throw new Error("Design Release requires production Gate A (qualification_fixture=false, real_source_source_fidelity=PASS, visual_file_verification=PASS)");
  ...
  if (gateBReport.production_gate !== true) throw new Error("Design Release requires production Gate B (production_gate must be true)");
