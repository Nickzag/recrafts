...
test("real Chromium evidence binds computed specimen styles and three viewport screenshots", async () => {
  ...
  const desktop = evidence.computed_styles.desktop;
  assert.ok(desktop, "desktop viewport styles missing");
  assert.equal(desktop.root_tokens["--recrafts-color-surface"], "#f7f7f5");
  assert.equal(desktop.specimen.borderRadius, "8px");
  assert.equal(desktop.specimen.borderTopWidth, "1px");
