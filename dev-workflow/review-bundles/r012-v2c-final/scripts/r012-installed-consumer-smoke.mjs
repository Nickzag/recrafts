...
export async function runInstalledConsumerSmoke({ workspace, cliFile, packageRoot }) {
  ...
  const gateB = await evaluateDesignCoherence({ candidate, designSource, previewHtml: rendered.html });
  ...
  const execute = (operation, input, expectedStatus, outputDirectory = undefined) => {
    ...
    const qualification = response.status === expectedStatus && (!response.validation || !response.validation.verdict || response.validation.verdict !== "FAIL") && ((expectedStatus === "failed" && exitCode !== 0) || (expectedStatus !== "failed" && exitCode === 0)) ? "PASS" : "FAIL";
  ...
  await writeFile(path.join(workspace, "browser-evidence.json"), JSON.stringify({ schema: "recrafts.browser-evidence/v1", status: "PASS", preview_sha256: sha256(previewResult.html), screenshots: { desktop: { file: path.join(workspace, "desktop.png"), sha256: desktopHash }, compact: { file: path.join(workspace, "compact.png"), sha256: compactHash }, mobile: { file: path.join(workspace, "mobile.png"), sha256: mobileHash } }, computed_styles: { root_tokens: {} }, evidence_sha256: bevHash }));
  await writeFile(path.join(workspace, "manifest.json"), JSON.stringify({ allowed_inputs: ["design.md", "task.md"] }));
  await writeFile(path.join(workspace, "candidates.json"), JSON.stringify({ runs: [{ candidate_id: "C1", run_id: "r1", agent: "test", target_lock: "static-coffee-v1", evidence_revision: "E-INSTALLED", allowed_inputs: ["design.md", "task.md"], accessed_inputs: ["design.md", "task.md"], output_sha256: "a".repeat(64) }] }));
  ...
  execute("compare-design-candidates", { candidates_file: "candidates.json", canonical_allowed_input_manifest: "manifest.json", target_lock: "static-coffee-v1", evidence_revision: "E-INSTALLED" }, "completed");
  ...
  return {
    ...
    tests: { passed: operationResults.filter((r) => r.qualification === "PASS").length, failed, total: operationResults.length, reporter: "installed-smoke" },
