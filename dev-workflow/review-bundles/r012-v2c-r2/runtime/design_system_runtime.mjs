...
export async function executeDesignOperation(request, workingRoot) {
  ...
  if (request.operation === "validate-design") {
    ...
    return { status: "completed", artifacts: [], validation: evaluateDesignCoherence({ candidate, designSource, previewHtml, browserEvidence, productionGate: true }) };
  }
  ...
  if (request.operation === "verify-source-fidelity") {
    ...
    const report = await evaluateSourceFidelity({ candidate, artifacts, visualFiles: Object.keys(visualFiles).length ? visualFiles : null }, { productionGate: input.production_gate === true });
  ...
  if (request.operation === "compare-design-candidates") {
    ...
    const targetLock = input.target_lock ?? null;
    const evidenceRev = input.evidence_revision ?? null;
    if (targetLock) {
      const runs = candidates.runs ?? candidates;
      for (const run of runs) {
        if (run.target_lock && run.target_lock !== targetLock) throw Object.assign(new Error(`Target lock mismatch: expected ${targetLock}, got ${run.target_lock}`), { code: "TARGET_LOCK_MISMATCH" });
        if (run.evidence_revision && run.evidence_revision !== evidenceRev) throw Object.assign(new Error(`Evidence revision mismatch: expected ${evidenceRev}, got ${run.evidence_revision}`), { code: "EVIDENCE_REVISION_MISMATCH" });
      }
    }
    const isolation = validateCandidateIsolation(candidates.runs ?? candidates, manifest ? { canonicalAllowedInputManifest: manifest, manifestSha256: manifestHash } : {});
    ...
    const comparison = compareDesignCandidates(candidates);
