...
export async function executeDesignOperation(request, workingRoot) {
  ...
  if (request.operation === "validate-design") {
    ...
    if (input.browser_evidence_file) {
      ...
      browserEvidence._evidence_dir = path.dirname(bevFile);
    }
    validateDesign(parseDesignMd(designSource));
    return { status: "completed", artifacts: [], validation: await evaluateDesignCoherence({ candidate, designSource, previewHtml, browserEvidence, productionGate: true }) };
  }
