...
export async function runInstalledConsumerSmoke({ workspace, cliFile, packageRoot }) {
  ...
  await writeFile(path.join(workspace, "browser-evidence.json"), JSON.stringify({ status: "PASS", preview_sha256: sha256(previewResult.html), screenshots: { desktop: { file: "desktop.png", sha256: "0".repeat(64) }, compact: { file: "compact.png", sha256: "0".repeat(64) }, mobile: { file: "mobile.png", sha256: "0".repeat(64) } }, computed_styles: { root_tokens: {} } }));
  await writeFile(path.join(workspace, "manifest.json"), JSON.stringify({ allowed_inputs: ["design.md", "task.md"] }));
  // candidates.json already written above with proper Semantic Compare format
