...
export async function createDesignRelease({ storeDirectory, releaseId, version, parentReleaseId, candidate, designSource, gateAReport, gateBReport, decisionReceipt }) {
  ...
  const gateAProd = gateAReport.qualification_fixture === false && gateAReport.real_source_source_fidelity === "PASS" && gateAReport.checks.visual_file_verification === "PASS";
  const gateAFixture = gateAReport.qualification_fixture === true && gateAReport.gate_a_runtime_qualification === "PASS";
  if (!gateAProd && !gateAFixture) throw new Error("Design Release requires either production Gate A (qualification_fixture=false, real_source_source_fidelity=PASS, visual_file_verification=PASS) or qualification fixture Gate A (qualification_fixture=true, gate_a_runtime_qualification=PASS)");
