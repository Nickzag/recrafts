  ...
  try {
    ...
    if (browserEvidence && browserEvidence.status === "PASS" && browserEvidence.screenshots && Object.keys(browserEvidence.screenshots).length >= 3) {
      ...
      for (const [name, shot] of Object.entries(browserEvidence.screenshots)) {
        if (shot.file) {
          try {
            const resolvedFile = path.isAbsolute(shot.file) ? shot.file : path.join(browserEvidence._evidence_dir || process.cwd(), shot.file);
    ...
    if (browserEvidenceValid && browserEvidence.computed_styles) {
        ...
        for (const token of allTokens) {
          ...
          if (browserEvidence.computed_styles.root_tokens && cssVar in browserEvidence.computed_styles.root_tokens && token.value !== undefined) {
