...
export async function evaluateDesignCoherence({ candidate, designSource, previewHtml, browserEvidence = null, productionGate = false }) {
  const failures = [];
  const checks = {};
  ...
  try {
    ...
    if (browserEvidenceValid && browserEvidence.computed_styles) {
        const tokenMap = new Map(Object.values(compileDesignIr(parseDesignMd(designSource))).flatMap((v) => Array.isArray(v) ? v : []).filter((t) => t && t.id).map((t) => [t.id, t]));
        for (const [tokenId, token] of tokenMap) {
          const cssVar = `--recrafts-${tokenId.replace(/[^a-zA-Z0-9_-]/g, "-")}`;
          if (browserEvidence.computed_styles.root_tokens && browserEvidence.computed_styles.root_tokens[cssVar] !== undefined && token.value !== undefined) {
            if (browserEvidence.computed_styles.root_tokens[cssVar] !== String(token.value)) { browserEvidenceValid = false; break; }
          }
        }
      }
