...
async function cleanupVersionReservation(root, releasePath, versionPath) {
  ...
  try { await writeIndex(root); } catch {}
}
...
export async function createDesignRelease({ storeDirectory, releaseId, version, parentReleaseId, candidate, designSource, gateAReport, gateBReport, decisionReceipt }) {
  ...
  if (gateAReport.qualification_fixture !== false) throw new Error("Design Release requires non-fixture Gate A (qualification_fixture must be false)");
  if (gateAReport.gate_a_runtime_qualification !== "PASS" || gateAReport.real_source_source_fidelity !== "PASS") throw new Error("Design Release requires production Gate A (gate_a_runtime_qualification and real_source_source_fidelity must be PASS)");
  if (!gateAReport.checks.visual_file_verification || gateAReport.checks.visual_file_verification !== "PASS") throw new Error("Design Release requires visual_file_verification PASS");
  if (!gateBReport.checks.browser_bound_visual || gateBReport.checks.browser_bound_visual !== "PASS") throw new Error("Design Release requires browser_bound_visual PASS");
  if (!gateBReport.browser_evidence_sha256) throw new Error("Design Release requires bound browser evidence hash");
  ...
  const artifacts = {
    ...
    "gate-a.json": gateAReport, "gate-b.json": gateBReport, [`${decisionReceipt.decision_id}.receipt.json`]: decisionReceipt
  };
  const releaseBody = {
