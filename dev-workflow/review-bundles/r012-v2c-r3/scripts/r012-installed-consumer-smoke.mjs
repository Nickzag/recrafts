...
export async function runInstalledConsumerSmoke({ workspace, cliFile, packageRoot }) {
  ...
  await writeFile(path.join(workspace, "browser-evidence.json"), JSON.stringify({ status: "PASS", preview_sha256: "0".repeat(64), screenshots: { desktop: {}, compact: {}, mobile: {} }, computed_styles: { root_tokens: {} } }));
  await writeFile(path.join(workspace, "manifest.json"), JSON.stringify({ allowed_inputs: ["design.md", "task.md"] }));
  await writeFile(path.join(workspace, "candidates.json"), JSON.stringify({ runs: [{ candidate_id: "C1", run_id: "r1", agent: "test", target_lock: "fixture", evidence_revision: "E1", allowed_inputs: ["design.md", "task.md"], accessed_inputs: ["design.md", "task.md"], output_sha256: "a".repeat(64) }] }));
