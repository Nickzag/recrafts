...
export async function executeDesignOperation(request, workingRoot) {
  ...
  if (request.operation === "compare-design-candidates") {
    ...
    const runs = input.candidate_runs_file ? await readJson(await inputFile(input.candidate_runs_file, workingRoot)) : null;
    ...
    const isolationRuns = runs || (Array.isArray(candidates) ? null : candidates.runs);
