...
export async function importDesignOwnerDecision({ decisionFile, candidate, gateAReport, gateBReport, outputDirectory, importedAt = new Date().toISOString() }) {
  ...
  const receiptFile = path.join(outputDirectory, `${decision.decision_id}.receipt.json`);
  if (await exists(receiptFile)) throw new Error(`Owner Decision Receipt already exists: ${decision.decision_id}`);
  const temporary = path.join(outputDirectory, `.${decision.decision_id}.receipt.next.json`);
