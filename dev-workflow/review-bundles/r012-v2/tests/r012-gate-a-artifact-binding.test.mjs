...
async function fixture(overrides = {}) {
  ...
  const files = {
    ...
    reconstruction: { schema: "recrafts.faithful-reconstruction/v1", candidate_revision: "C1", design_sha256: "d".repeat(64), surfaces: [{ region_id: "RG1", status: "rendered" }] },
    comparison_report: { schema: "recrafts.source-comparison/v1", candidate_revision: "C1", comparisons: [{ region_id: "RG1", visual_similarity: 0.97, unsupported_objects: [], unknowns_hardened: false }] },
    geometry_report: { schema: "recrafts.geometry-report/v1", candidate_revision: "C1", regions: [{ region_id: "RG1", delta_px: 1, tolerance_px: 2 }] }
  };
...
test("Gate A fails closed on stale hashes and cross-artifact identity mismatches", async () => {
  ...
  stale.artifacts.region_set.sha256 = "0".repeat(64);
  ...
  for (const [key, override] of [
    ...
    ["reconstruction", { design_sha256: "e".repeat(64) }]
  ]) {
...
test("Gate A derives FAIL from missing coverage, unsupported objects, geometry mismatch, and hardened unknowns", async () => {
  const cases = [
    { region_set: { regions: [] } },
    { comparison_report: { comparisons: [{ region_id: "RG1", visual_similarity: 0.97, unsupported_objects: ["floating badge"], unknowns_hardened: false }] } },
    { comparison_report: { comparisons: [{ region_id: "RG1", visual_similarity: 0.97, unsupported_objects: [], unknowns_hardened: true }] } },
