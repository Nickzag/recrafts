...
const gate = await import("../runtime/design_coherence_gate.mjs").catch(() => ({}));
...
test("Gate B derives a bound PASS from deep IR and actual Preview coverage", () => {
  assert.equal(typeof gate.evaluateDesignCoherence, "function");
  const preview = renderDesignPreview({ designSource: canonical, generatedAt: "2026-08-09T00:00:00.000Z" });
  const report = gate.evaluateDesignCoherence({ candidate: candidate(canonical), designSource: canonical, previewHtml: preview.html });
  ...
  assert.ok(Object.values(report.checks).every((status) => status === "PASS"));
});

test("Gate B rejects incomplete visual grammar, unknown governance, accessibility, and responsive semantics", () => {
  const mutations = [
  ...
  for (const source of mutations) {
    ...
    const report = gate.evaluateDesignCoherence({ candidate: candidate(source), designSource: source, previewHtml: preview.html });
    assert.equal(report.verdict, "FAIL");
    assert.ok(report.failures.length > 0);
  }
});

test("Gate B rejects stale Candidate and Preview bindings", () => {
  const preview = renderDesignPreview({ designSource: canonical, generatedAt: "2026-08-09T00:00:00.000Z" });
  assert.equal(gate.evaluateDesignCoherence({ candidate: { ...candidate(canonical), design_sha256: "0".repeat(64) }, designSource: canonical, previewHtml: preview.html }).verdict, "FAIL");
  assert.equal(gate.evaluateDesignCoherence({ candidate: candidate(canonical), designSource: canonical, previewHtml: preview.html.replace("Document title", "Tampered") }).verdict, "FAIL");
});
