...
export const DESIGN_OPERATIONS = ["parse-design-md", "compile-design-ir", "validate-design", "render-design-preview", "compare-design-candidates", "import-design-owner-decision", "create-design-release", "validate-design-release", "rollback-design-release", "compare-design-releases"];
...
export async function executeDesignOperation(request, workingRoot) {
  ...
  if (request.operation === "validate-design") {
    ...
    return { status: "completed", artifacts: [], validation: evaluateDesignCoherence({ candidate, designSource, previewHtml }) };
  }
  if (request.operation === "compare-design-candidates") {
    const file = await inputFile(input.candidates_file, workingRoot);
    return { status: "completed", artifacts: [], validation: compareDesignCandidates(await readJson(file)) };
  }
