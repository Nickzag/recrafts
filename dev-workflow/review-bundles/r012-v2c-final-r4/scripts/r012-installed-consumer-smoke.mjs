...
export async function runInstalledConsumerSmoke({ workspace, cliFile, packageRoot }) {
  ...
  await writeFile(path.join(workspace, "browser-evidence.json"), JSON.stringify({ schema: "recrafts.browser-evidence/v1", status: "PASS", preview_sha256: sha256(previewResult.html), screenshots: { desktop: { file: "desktop.png", sha256: desktopHash, viewport: { width: 1440, height: 1000 } }, compact: { file: "compact.png", sha256: compactHash, viewport: { width: 768, height: 1000 } }, mobile: { file: "mobile.png", sha256: mobileHash, viewport: { width: 390, height: 844 } } }, computed_styles: { root_tokens: {} }, infrastructure_selector_audit: "PASS", infrastructure_affects_visual_region: false }));
  ...
  await writeFile(path.join(workspace, "candidate-runs.json"), JSON.stringify([{ candidate_id: "C1", run_id: "r1", agent: "test", target_lock: "static-coffee-v1", evidence_revision: "E-INSTALLED", allowed_inputs: ["design.md", "task.md"], accessed_inputs: ["design.md", "task.md"], output_sha256: "a".repeat(64) }]));
