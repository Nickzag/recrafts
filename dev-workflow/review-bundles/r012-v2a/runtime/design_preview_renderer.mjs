...
function componentStyles(ir) {
  return ir.components.flatMap((component) => {
    ...
    const base = `${selector}{${bindingCss(component.specimen.token_bindings)};border-style:solid;display:grid;grid-template-columns:repeat(${component.anatomy.length},minmax(0,1fr));align-items:center}`;
...
function renderComposition(composition, components, viewport = "desktop") {
  ...
  const regions = composition.layout.region_order.map((regionId) => composition.regions.find((region) => region.id === regionId)).filter(Boolean).map((region) => {
    ...
    return `<section data-recrafts-region="${esc(region.id)}"><h3>${esc(region.role)}</h3>${specimens}</section>`;
  }).join("");
