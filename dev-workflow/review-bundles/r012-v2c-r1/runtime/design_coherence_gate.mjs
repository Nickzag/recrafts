...
export function evaluateDesignCoherence({ candidate, designSource, previewHtml, browserEvidence = null, productionGate = false }) {
  ...
  try {
    ...
    const hasBrowserEvidence = browserEvidence && browserEvidence.status === "PASS" && browserEvidence.screenshots && Object.keys(browserEvidence.screenshots).length >= 3;
    checks.browser_bound_visual = hasBrowserEvidence ? "PASS" : productionGate ? "FAIL" : "NOT_RUN";
  ...
  const report = {
    ...
    checks, failures: [...new Set(failures)], proves_source_fidelity: false, browser_evidence_sha256: browserEvidence?.evidence_sha256 ?? null
  };
