...
export async function evaluateSourceFidelity({ candidate, artifacts, visualFiles = null }, { qualificationFixture = false, productionGate = false } = {}) {
  ...
  let visualFailures = [];
  if (productionGate && !visualFiles) { visualFailures.push("production-gate-requires-visual-files"); checks.visual_file_verification = "FAIL"; }
  if (visualFiles) {
    try {
