...
const VERSIONS = { skill: "0.3.0-preflight", runtime: "r002-extraction-v2", schema: "2.1.0", capture_adapter: "capture-v2" };
const PROMPT_HASH = createHash("sha256").update("recrafts-host-agent-extraction-contract-v1").digest("hex");
...
function identity(kind, value) { return `${kind}-${sha256(Buffer.from(JSON.stringify(value))).slice(0, 16)}`; }
function capabilityHandshake(capabilities, hasClassifiedEvidence) {
  const provided = new Set(capabilities);
  const required = ["files", "structured-output"];
  const recommended = ["vision", "browser", "screenshot"];
  const missingRequired = required.filter((item) => !provided.has(item));
  const missingRecommended = recommended.filter((item) => !provided.has(item));
  const visionBlocked = !provided.has("vision") && !hasClassifiedEvidence;
  return {
    required_capabilities: required,
    recommended_capabilities: recommended,
    provided_capabilities: [...provided].sort(),
    missing_capabilities: [...missingRequired, ...missingRecommended],
    status: missingRequired.length || visionBlocked ? "blocked" : missingRecommended.length ? "degraded" : "ready",
    next_action: missingRequired.length ? "Use a host with file and structured-output capabilities." : visionBlocked ? "Use a vision-capable host agent or provide an existing evidence map." : missingRecommended.length ? "Continue with declared degraded-mode limits." : "Continue."
  };
}
function isPrivateAddress(address) {
  return /^(?:127\.|10\.|192\.168\.|169\.254\.|0\.|::1$|fc|fd|fe80)|^172\.(?:1[6-9]|2\d|3[01])\./i.test(address);
}
async function assertPublicTarget(url) {
  ...
  if (["localhost", "::1"].includes(url.hostname)) throw new Error("Private-network website capture is not allowed");
  const addresses = await lookup(url.hostname, { all: true });
  if (!addresses.length || addresses.some(({ address }) => isPrivateAddress(address))) throw new Error("Private-network website capture is not allowed");
  if (/\.(?:dmg|exe|zip|pkg|msi|pdf)$/i.test(url.pathname) || /checkout|subscription|download/i.test(url.pathname)) throw new Error("Download or checkout URL is outside capture scope");
}
async function fetchPublicPage(initialUrl, origin) {
  let current = new URL(initialUrl);
  for (let redirects = 0; redirects <= 3; redirects += 1) {
    await assertPublicTarget(current);
    if (current.origin !== origin && !(current.protocol === "https:" && current.hostname === new URL(origin).hostname)) throw new Error("Cross-origin redirect is not allowed");
    const response = await fetch(current, { redirect: "manual", signal: AbortSignal.timeout(15000), headers: { "user-agent": "Recrafts-MVP-Evidence-Capture/0.3" } });
    if (response.status >= 300 && response.status < 400) {
      ...
      const httpUpgrade = current.protocol === "http:" && next.protocol === "https:" && sameHost;
      if (!sameHost || (!httpUpgrade && next.origin !== current.origin)) throw new Error("Cross-origin redirect is not allowed");
      current = next;
      continue;
    }
...
async function loadImages(inputPath, single) {
  ...
  for (const [index, file] of resolvedPaths.entries()) {
    ...
    const source = {
      source_id: prior?.source_id ?? `source-${hash.slice(0, 12)}`,
  ...
  return sources.sort((a, b) => a.sha256.localeCompare(b.sha256));
}
...
async function exportDraft(output, sources, extracted, mode, identityContext = {}) {
  const captureConfig = { mode, source_hashes: sources.map((source) => source.sha256).sort(), capture_adapter_version: VERSIONS.capture_adapter, viewport: identityContext.viewport ?? null, routes: identityContext.routes ?? [] };
  const captureId = identity("capture", captureConfig);
  const analysisConfig = identityContext.analysis_config ?? {};
  const analysisIdentity = { capture_id: captureId, skill_version: VERSIONS.skill, runtime_version: VERSIONS.runtime, schema_version: VERSIONS.schema, prompt_hash: PROMPT_HASH, host_agent: identityContext.host_agent ?? "unavailable", model: identityContext.model ?? "unavailable", configuration_hash: sha256(Buffer.from(JSON.stringify(analysisConfig))) };
  const analysisId = identity("analysis", analysisIdentity);
  const packageIdentity = { analysis_id: analysisId, human_decision_set: identityContext.human_decision_set ?? "none", correction_version: identityContext.correction_version ?? 0 };
  const packageId = identity("package", packageIdentity);
  const handshake = capabilityHandshake(identityContext.host_capabilities ?? ["files", "structured-output"], sources.some((source) => source.regions.some((region) => region.class !== "unknown")));
  if (handshake.status === "blocked") throw new Error(`Host capability handshake blocked: ${handshake.next_action}`);
  const manifest = { version: VERSIONS.schema, capture_id: captureId, analysis_id: analysisId, package_id: packageId, mode, skill_version: VERSIONS.skill, runtime_version: VERSIONS.runtime, schema_version: VERSIONS.schema, capture_adapter_version: VERSIONS.capture_adapter, host_agent: analysisIdentity.host_agent, model: analysisIdentity.model, prompt_hash: PROMPT_HASH, configuration_hash: analysisIdentity.configuration_hash, sources };
  ...
  await writeJson(path.join(output, "capability-handshake.json"), handshake);
  ...
  await writeJson(path.join(output, "run-log.json"), { capture_id: captureId, analysis_id: analysisId, package_id: packageId, mode, stages: ["intake","capture","normalize","classify","extract","compose","validate","export-draft"], inputs: sources.map((source) => ({ source_id: source.source_id, sha256: source.sha256 })) });
  await saveStage(output, 8, "export-draft", { package_id: packageId });
  return { capture_id: captureId, analysis_id: analysisId, package_id: packageId, output };
}

export async function analyzeImages({ input, output, single = false, hostAgent, model, hostCapabilities }) {
  const target = await prepareOutput(output);
  const sources = await loadImages(input, single);
  return exportDraft(target, sources, extract(sources, single ? "single-image" : "multi-image"), single ? "single-image" : "multi-image", { host_agent: hostAgent, model, host_capabilities: hostCapabilities });
}

export async function analyzeWebsite({ url, output, routes = [], action, screenshot, hostAgent, model, hostCapabilities }) {
  ...
  try { ({ response, finalUrl } = await fetchPublicPage(url, parsed.origin)); }
  catch (error) { throw new Error(`Website inaccessible or unsafe redirect: ${error.message}`); }
  ...
  for (const route of routeUrls) {
    let routeResponse;
    try { ({ response: routeResponse } = await fetchPublicPage(route, parsed.origin)); }
    catch (error) { throw new Error(`Website route inaccessible or unsafe redirect: ${route.pathname}: ${error.message}`); }
  ...
  const capture = { url, final_url: finalUrl.href, status: response.status, viewport: { width: 1440, height: 900 }, html_sha256: sha256(Buffer.from(html)), routes: routeMetadata, dom_summary: { title: html.match(/<title[^>]*>([^<]*)/i)?.[1] ?? "", headings: [...html.matchAll(/<h[1-3][^>]*>(.*?)<\/h[1-3]>/gis)].slice(0, 30).map((match) => match[1].replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim()) }, css_variable_candidates: cssVariables, assets_manifest: assets, screenshot: screenshot ? path.basename(screenshot) : null };
  ...
  return exportDraft(target, [source], extract([source], "public-website"), "public-website", { host_agent: hostAgent, model, host_capabilities: hostCapabilities, viewport: capture.viewport, routes: routeMetadata.map((item) => item.url) });
}
