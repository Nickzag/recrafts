...
test("npm dry-run inventory contains interop runtime and excludes repository evidence", () => {
  ...
  for (const required of ["runtime/interop_cli.mjs","contracts/host-analysis.schema.json","fixtures/interop/host-analysis.fixture.json"]) assert.ok(files.includes(required), required);
...
test("generated RC passes readiness when present", async (context) => {
  const directory = path.join(root, "release-candidates/recrafts-0.3.0-rc.1-build4");
